...
async function loadLiveContent() {
  try {
    ...
    if (articles.length && journal) journal.innerHTML = articles.slice(0, 4).map(item => `<article class="article"><img src="${escapeHtml(item.image_url || '/assets/articles/wellness-rituale-serale.png')}" alt="${escapeHtml(item.title)}"><div><span class="tag">${escapeHtml(item.category)}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.excerpt || item.meta_description)}</p><a href="#newsletter">Leggi presto →</a></div></article>`).join('');
